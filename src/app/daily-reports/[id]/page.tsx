...
export default function DailyReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadSchedules() {
      ...
      const { data } = await supabase.from('schedules').select('*')
        .eq('user_id', userId)
        .gte('start_datetime', date + 'T00:00:00+09:00')
        .lte('start_datetime', date + 'T23:59:59+09:00')
  ...
  function hasRowContent(d: DailyReportDetail): boolean {
              ...
              {details.map((d, i) => (
                <tr key={i}>
                  ...
                  <td className="px-1 py-1">
                    <select className={`w-full border rounded px-1 py-1 text-xs ${!d.task_type && hasRowContent(d) ? 'border-red-400 bg-red-50' : 'border-gray-200'}`} value={d.task_type || ''} onChange={e => setRow(i, 'task_type', e.target.value)}>
                    ...
                    {d.task_type && YEAR_TASKS.includes(d.task_type) ? (
                      <div className="flex items-center gap-1">
                        <input type="number" placeholder="2026" min="2000" max="2100"
                          ...
                          value={d.subject || ''} onChange={e => setRow(i, 'subject', e.target.value)} />
                        <span className="text-gray-400 text-xs shrink-0">年度</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-0.5">
                        <MonthPartInput value={d.subject || ''} onChange={v => setRow(i, 'subject', v)}
                        ...
                        {!(d.task_type && SINGLE_MONTH_TASKS.includes(d.task_type)) && (
                          <>
                            ...
                            <MonthPartInput value={d.details || ''} onChange={v => setRow(i, 'details', v)} />
                          </>
                        )}
                      </div>
                    )}
